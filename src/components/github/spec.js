...
test("Github renders correctly", () => {
  const tree = renderer.create(<Github {...color.red} />).toJSON();
...
test("Github onChange events correctly", () => {
  ...
  const tree = mount(<Github onChange={changeSpy} />);
...
test("Github with onSwatchHover events correctly", () => {
  ...
  const tree = mount(<Github onSwatchHover={hoverSpy} />);
...
test('Github `triangle="hide"`', () => {
  const tree = renderer
    .create(<Github {...color.red} triangle="hide" />)
    .toJSON();
  expect(tree).toMatchSnapshot();
});
...
test('Github `triangle="top-right"`', () => {
  const tree = renderer
    .create(<Github {...color.red} triangle="top-right" />)
    .toJSON();
  expect(tree).toMatchSnapshot();
});
...
test("Github renders custom styles correctly", () => {
  const tree = renderer
    .create(
      <Github
        {...color.red}
        styles={{ default: { card: { boxShadow: "0 0 10px red" } } }}
      />
    )
    .toJSON();
...
test("GithubSwatch renders correctly", () => {
  const tree = renderer.create(<GithubSwatch color="#333" />).toJSON();
  expect(tree).toMatchSnapshot();
});
