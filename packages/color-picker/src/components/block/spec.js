...
test("Block onChange events correctly", () => {
  ...
  const tree = mount(<Block onChange={changeSpy} />);
...
test("Block with onSwatchHover events correctly", () => {
  ...
  const tree = mount(<Block onSwatchHover={hoverSpy} />);
...
test('Block `triangle="hide"`', () => {
  const tree = renderer.create(<Block triangle="hide" />).toJSON();
  expect(tree).toMatchSnapshot();
});

test("BlockSwatches renders correctly", () => {
  const tree = renderer
    .create(<BlockSwatches colors={["#fff", "#999", "#000"]} />)
    .toJSON();
  expect(tree).toMatchSnapshot();
});
test("Block renders custom styles correctly", () => {
  const tree = renderer
    .create(<Block styles={{ default: { card: { boxShadow: "none" } } }} />)
    .toJSON();
