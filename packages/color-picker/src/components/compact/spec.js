...
test("Compact renders correctly", () => {
  const tree = renderer.create(<Compact {...color.red} />).toJSON();
  expect(tree).toMatchSnapshot();
});

test("Compact with onSwatchHover renders correctly", () => {
  const tree = renderer
    .create(<Compact {...color.red} onSwatchHover={() => {}} />)
    .toJSON();
  expect(tree).toMatchSnapshot();
});
...
test("Compact onChange events correctly", () => {
  ...
  const tree = mount(<Compact {...color.red} onChange={changeSpy} />);
...
test("Compact with onSwatchHover events correctly", () => {
  ...
  const tree = mount(<Compact {...color.red} onSwatchHover={hoverSpy} />);
...
test("CompactColor renders correctly", () => {
  const tree = renderer.create(<CompactColor />).toJSON();
  expect(tree).toMatchSnapshot();
});

test("CompactFields renders correctly", () => {
  const tree = renderer.create(<CompactFields {...color.red} />).toJSON();
  expect(tree).toMatchSnapshot();
});
...
test("Compact renders custom styles correctly", () => {
  const tree = renderer
    .create(
      <Compact
        {...color.red}
        styles={{ default: { wrap: { boxShadow: "0 0 10px red" } } }}
      />
    )
    .toJSON();
